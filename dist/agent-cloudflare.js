 ...
 */
export class PocketBaseMCPAgent extends Agent {
    ...
    setupDiscoveryTools() {
        // Health check tool
        this.server.tool("health_check", "Check the health status of the MCP server and PocketBase connection", {}, async () => {
            const status = {
                server: 'healthy',
        ...
        this.server.tool("smithery_discovery", "Fast discovery endpoint for Smithery tool scanning", {}, async () => {
            return {
                content: [{
                        type: 'text',
                        text: JSON.stringify({
        ...
        // Tool discovery
        this.server.tool("discover_tools", "List all available tools and their current status", {}, async () => {
            const tools = [
    ...
    setupPocketBaseTools() {
        // List collections
        this.server.tool("list_collections", "List all collections in the PocketBase database", {}, async () => {
            if (!this.pb) {
        ...
        // Get collection
        this.server.tool("get_collection", "Get details of a specific collection", { nameOrId: z.string().describe('Collection name or ID') }, async ({ nameOrId }) => {
            if (!this.pb) {
        ...
        // Create collection
        this.server.tool("create_collection", "Create a new collection in the database", {
        ...
        // List records
        this.server.tool("list_records", "List records from a collection with optional filtering and pagination", {
        ...
        // Get record
        this.server.tool("get_record", "Get a specific record by ID", {
            collection: z.string().describe('Collection name'),
        ...
        // Create record
        this.server.tool("create_record", "Create a new record in a collection", {
            collection: z.string().describe('Collection name'),
        ...
        // Update record
        this.server.tool("update_record", "Update an existing record", {
            collection: z.string().describe('Collection name'),
            id: z.string().describe('Record ID'),
        ...
        // Delete record
        this.server.tool("delete_record", "Delete a record from a collection", {
            collection: z.string().describe('Collection name'),
            id: z.string().describe('Record ID')
        }, async ({ collection, id }) => {
            if (!this.pb) {
        ...
        // Query records
        this.server.tool("query_records", "Perform complex queries on records with advanced filtering", {
            collection: z.string().describe('Collection name'),
            filter: z.string().optional().describe('Filter expression'),
        ...
        // Authenticate user
        this.server.tool("authenticate_user", "Authenticate a user with email/username and password", {
    ...
    setupUtilityTools() {
        // Set custom header
        this.server.tool("set_custom_header", "Set a custom header for PocketBase requests", {
    ...
    setupStripeTools() {
        if (!this.stripeService)
            return;
        this.server.tool("create_customer", "Create a new customer in Stripe", {
        ...
        this.server.tool("create_payment_intent", "Create a payment intent in Stripe", {
    ...
    setupEmailTools() {
        if (!this.emailService)
            return;
        this.server.tool("send_email", "Send an email using the configured email service", {
    ...
    setupResources() {
        ...
        this.server.resource("agent_status", "agent://status", async (uri) => {
            return {
                contents: [{
                        uri: uri.href,
        ...
        this.server.resource("database_stats", "pocketbase://stats", async (uri) => {
            if (!this.pb) {
                return {
                    contents: [{
                            uri: uri.href,
    ...
    setupPrompts() {
        ...
        this.server.prompt("pocketbase_setup", "Guide for setting up PocketBase MCP server", async () => {
...
The server will automatically initialize once properly configured!`
        ...
        this.server.prompt("database_schema_design", "Help design PocketBase database schema", async () => {
            return {
