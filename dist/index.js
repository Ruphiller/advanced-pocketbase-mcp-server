...
class PocketBaseServer {
    server;
    pb; // Make optional for deferred initialization
    _customHeaders = {};
    _realtimeSubscriptions = new Map();
    stripeService;
    emailService;
     ...
     */
    async initializePocketBase(config) {
     ...
     */
    async authenticatePocketBase(email, password, isAdmin = false) {
        if (!this.pb) {
        ...
        try {
            ...
            if (isAdmin) {
                try {
                    ...
                    await this.pb.admins.authWithPassword(authEmail, authPassword);
                }
                catch (error) {
                    if (error.status === 400) {
    ...
    async doInitialization(options) {
    ...
    setupPrompts() {
        ...
        this.server.prompt("create-collection", "Create a new collection with comprehensive schema design", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Please specify the collection name, intended purpose, and required fields.`
        ...
        this.server.prompt("create-record", "Create a new record with proper data validation", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Specify the target collection and the data you want to store.`
        ...
        this.server.prompt("build-query", "Build advanced queries with filtering, sorting, and relationships", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Describe your query requirements including collection, filters, sorting needs, and any relationships to expand.`
        ...
        this.server.prompt("setup-saas-backend", "Set up a complete SaaS backend with user management, payments, and email templates", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Specify your business type, required features, and any specific customizations needed.`
        ...
        this.server.prompt("create-subscription-plan", "Create a comprehensive subscription plan with pricing and features", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Describe your subscription model, target pricing, and feature differentiation.`
        ...
        this.server.prompt("setup-user-onboarding", "Design a comprehensive user onboarding flow with email sequences", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Describe your product, target users, and key activation goals for the onboarding flow.`
        ...
        this.server.prompt("create-payment-workflow", "Set up payment processing with webhooks and email notifications", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Describe your payment model, required integrations, and specific business requirements.`
        ...
        this.server.prompt("design-schema", "Design a complete database schema for a specific business domain", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Describe your business domain, main entities, and their relationships.`
        ...
        // Relationship setup
        this.server.prompt("create-relationships", "Set up complex relationships between collections with proper access rules", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Describe the collections you want to relate and the business logic governing their relationships.`
        ...
        this.server.prompt("analytics-query", "Build analytics queries with aggregations, grouping, and metrics", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Describe the analytics you need, key metrics to track, and the business questions you want to answer.`
        ...
        this.server.prompt("email-campaign", "Design email campaigns with templates, scheduling, and tracking", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
...
Describe your campaign goals, target audience, and desired email sequence.`
    ...
    setupResources() {
        ...
        this.server.resource("server-info", "pocketbase://info", async (uri) => {
            try {
        ...
        this.server.resource("collection-schema", new ResourceTemplate("pocketbase://collections/{name}/schema", { list: undefined }), async (uri, params) => {
        ...
        this.server.resource("collections", "pocketbase://collections", async (uri) => {
            try {
                await this.ensureInitialized();
                ...
                const collections = {
                    ...
                    items: collectionsResponse.items
                };
                return {
                    contents: [{
                            uri: uri.href,
        ...
        // Record resource
        this.server.resource("record", new ResourceTemplate("pocketbase://collections/{collection}/records/{id}", { list: undefined }), async (uri, params) => {
        ...
        // Auth info resource
        this.server.resource("auth-info", "pocketbase://auth", async (uri) => {
            try {
                await this.ensureInitialized();
                return {
                    contents: [{
        ...
        this.server.resource("analytics-metrics", "analytics://metrics", async (uri) => {
            try {
                await this.ensureInitialized();
                const metrics = {
                    timestamp: new Date().toISOString(),
        ...
        this.server.resource("user-activity", "analytics://user-activity", async (uri) => {
            try {
                await this.ensureInitialized();
                const activity = {
                    timestamp: new Date().toISOString(),
                ...
                try {
                    ...
                    activity.real_time.recent_events = recentEvents.items.map((event) => ({
        ...
        this.server.resource("daily-summary", "reports://daily-summary", async (uri) => {
            try {
                await this.ensureInitialized();
                const today = new Date();
                today.setHours(0, 0, 0, 0);
                ...
                try {
                    ...
                    try {
                        ...
                        const revenue = paymentsToday.reduce((sum, payment) => {
        ...
        this.server.resource("revenue-trends", "reports://revenue-trends", async (uri) => {
            try {
                await this.ensureInitialized();
                const trends = {
                    generated_at: new Date().toISOString(),
        ...
        this.server.resource("email-templates", "config://email-templates", async (uri) => {
            try {
                await this.ensureInitialized();
                const config = {
                    timestamp: new Date().toISOString(),
                ...
                try {
                    ...
                    config.templates = templates.map((template) => ({
                        id: template.id,
                        name: template.name,
                    ...
                    config.missing_templates = config.required_templates.filter((required) => !existingNames.includes(required));
                    ...
                    config.summary = {
                        ...
                        invalid_templates: Object.values(config.template_validation).filter((v) => !v.is_valid).length,
        ...
        this.server.resource("stripe-products", "config://stripe-products", async (uri) => {
            try {
                await this.ensureInitialized();
                const config = {
                    timestamp: new Date().toISOString(),
                ...
                if (!this.stripeService) {
                    ...
                    try {
                        ...
                        config.products = localProducts.map((product) => ({
                            id: product.id,
                            name: product.name,
                        ...
                        if (config.products.length > 0) {
                            const recurringProducts = config.products.filter((p) => p.recurring);
                            const oneTimeProducts = config.products.filter((p) => !p.recurring);
                            config.pricing_analysis = {
                                ...
                                price_ranges: {
                                    lowest_price: Math.min(...config.products.map((p) => p.price)),
                                    highest_price: Math.max(...config.products.map((p) => p.price)),
                                    average_price: config.products.reduce((sum, p) => sum + p.price, 0) / config.products.length
                                },
                                intervals: {
                                    monthly: recurringProducts.filter((p) => p.interval === 'month').length,
                                    yearly: recurringProducts.filter((p) => p.interval === 'year').length,
                                    weekly: recurringProducts.filter((p) => p.interval === 'week').length
                                }
                            };
                        }
        ...
        this.server.resource("schema-docs", "dev://schema-docs", async (uri) => {
            try {
                await this.ensureInitialized();
                const docs = {
                    generated_at: new Date().toISOString(),
                ...
                try {
                    ...
                    docs.database_info = {
                        ...
                        system_collections: collections.items.filter((c) => c.system).length,
                        user_collections: collections.items.filter((c) => !c.system).length
                    };
                    docs.collections = collections.items.map((collection) => {
                        ...
                        return {
                            ...
                            schema: {
                                ...
                                relations: schema.filter((field) => field.type === 'relation').map((field) => ({
                    ...
                    // Indexes summary
                    const totalIndexes = docs.collections.reduce((sum, col) => sum + (col.indexes?.length || 0), 0);
                    docs.indexes_summary = {
                        total_indexes: totalIndexes,
                        collections_with_indexes: docs.collections.filter((col) => col.indexes && col.indexes.length > 0).length,
        ...
        this.server.resource("api-endpoints", "dev://api-endpoints", async (uri) => {
            try {
                await this.ensureInitialized();
                const endpoints = {
                    generated_at: new Date().toISOString(),
                ...
                try {
                    ...
                    endpoints.collections = collections.items
                        .filter((c) => !c.system) // Focus on user collections
                        .map((collection) => {
    ...
    analyzeSecurityLevel(collection) {
    ...
    analyzeRelationships(collections) {
        const relationships = [];
        collections.forEach(collection => {
            if (collection.schema?.relations) {
                collection.schema.relations.forEach((relation) => {
                    relationships.push({
    ...
    checkBidirectional(collections, fromCollection, toCollection) {
        const targetCollection = collections.find(c => c.name === toCollection);
        if (!targetCollection?.schema?.relations)
            return false;
        return targetCollection.schema.relations.some((rel) => rel.target_collection === fromCollection);
    }
    generateIndexRecommendations(collections) {
        const recommendations = [];
        collections.forEach(collection => {
            const hasDateFields = collection.schema?.fields?.some((f) => f.type === 'date');
            const hasRelations = collection.schema?.fields?.some((f) => f.type === 'relation');
    ...
    analyzeNamingConventions(collections) {
        const analysis = {
    ...
    analyzeSecurityPractices(collections) {
        const analysis = {
    ...
    generatePerformanceTips(collections) {
        const tips = [
    ...
    generateDataModelingSuggestions(collections) {
        const suggestions = [
    ...
    calculateHealthScore(metrics) {
        ...
        if (metrics.technical_metrics?.database_status !== 'Connected')
            score -= 50;
        if (!metrics.technical_metrics?.auth_status)
            score -= 20;
        if (metrics.user_metrics?.error)
            score -= 15;
        if (metrics.business_metrics?.info)
            score -= 10;
        return Math.max(0, score);
    }
    setupTools() {
        console.error('[MCP DEBUG] Setting up tools...'); // Simple test tool
        const testTool = this.server.tool('test_tool', {}, async () => {
        ...
        this.server.tool('discover_tools', {}, async () => {
        ...
        this.server.tool('smithery_discovery', {}, async () => {
        ...
        this.server.tool('list_registered_tools', {}, async () => {
        ...
        this.server.tool('get_auth_info', {}, async () => {
            try {
                await this.ensureInitialized();
                return {
                    content: [{
        ...
        this.server.tool('list_collections', {
            ...
            try {
                ...
                const filteredCollections = includeSystem
                    ? collections.items
                    : collections.items.filter((c) => !c.system);
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify(filteredCollections.map((c) => ({
                                id: c.id,
                                name: c.name,
                                type: c.type,
                                system: c.system,
        ...
        this.server.tool('list_records', {
            ...
            try {
                ...
                if (typeof page === 'number' && page < 1)
                    page = 1;
                if (typeof perPage === 'number' && perPage > 500)
                    perPage = 500;
        ...
        this.server.tool('update_record', {
        ...
        this.server.tool('delete_record', {
            collection: z.string().describe('Collection name where the record exists (e.g., "users", "posts", "products")'),
        ...
        this.server.tool('authenticate_user', {
        ...
        this.server.tool('authenticate_with_oauth2', {
        ...
        this.server.tool('authenticate_with_otp', {
        ...
        this.server.tool('request_verification', {
        ...
        this.server.tool('confirm_verification', {
        ...
        this.server.tool('request_password_reset', {
        ...
        this.server.tool('confirm_password_reset', {
        ...
        this.server.tool('request_email_change', {
        ...
        this.server.tool('confirm_email_change', {
        ...
        this.server.tool('impersonate_user', {
        ...
        this.server.tool('set_collection_rules', {
        ...
        const updateCollectionSchemaTool = this.server.tool('update_collection_schema', {
        ...
        this.server.tool('get_collection_schema', {
        ...
        this.server.tool('backup_database', {
        ...
        this.server.tool('migrate_collection', {
        ...
        this.server.tool('manage_indexes', {
            ...
            try {
                ...
                switch (action) {
                    ...
                    case 'delete':
                        ...
                        const filteredIndexes = currentIndexes.filter((idx) => idx.name !== index.name);
        ...
        this.server.tool('set_request_options', {
        ...
        this.server.tool('manage_auth_store', {
        ...
        this.server.tool('subscribe_to_collection', {
            ...
            try {
                ...
                await this.pb.collection(collection).subscribe(recordId || '*', (e) => {
        ...
        this.server.tool('batch_update_records', {
        ...
        this.server.tool('batch_delete_records', {
        ...
        this.server.tool('execute_batch_operations', {
        ...
        this.server.tool('setup_advanced_collections', {}, async () => {
            try {
                const collections = [
                    {
        ...
        this.server.tool('stripe_create_product', {
        ...
        this.server.tool('stripe_create_customer', {
        ...
        this.server.tool('stripe_create_checkout_session', {
        ...
        this.server.tool('stripe_create_payment_intent', {
        ...
        this.server.tool('stripe_retrieve_customer', {
        ...
        this.server.tool('stripe_update_customer', {
            customerId: z.string().describe('Stripe customer ID'),
        ...
        this.server.tool('stripe_cancel_subscription', {
        ...
        this.server.tool('list_stripe_products', {
        ...
        this.server.tool('list_stripe_customers', {
            page: z.number().optional().default(1).describe('Page number'),
            perPage: z.number().optional().default(50).describe('Records per page'),
        ...
        this.server.tool('list_stripe_subscriptions', {
            page: z.number().optional().default(1).describe('Page number'),
            perPage: z.number().optional().default(50).describe('Records per page'),
        ...
        this.server.tool('stripe_handle_webhook', {
        ...
        this.server.tool('sync_stripe_products', {}, async () => {
            try {
        ...
        this.server.tool('stripe_create_treasury_financial_account', {
        ...
        this.server.tool('stripe_create_climate_order', {
        ...
        this.server.tool('stripe_create_terminal_connection_token', {
        ...
        this.server.tool('stripe_create_issuing_card', {
        ...
        this.server.tool('stripe_create_app_secret', {
        ...
        this.server.tool('email_create_template', {
        ...
        this.server.tool('email_get_template', {
            name: z.string().describe('Template name')
        }, async ({ name }) => {
            try {
                if (!process.env.EMAIL_SERVICE && !process.env.SMTP_HOST) {
        ...
        this.server.tool('email_update_template', {
            name: z.string().describe('Template name'),
        ...
        this.server.tool('email_send_templated', {
            ...
            try {
                ...
                if (hasEnhancedFeatures && this.emailService.hasEnhancedFeatures()) {
                    ...
                    if (categories)
                        enhancedData.categories = categories;
                    if (customArgs)
                        enhancedData.customArgs = customArgs;
        ...
        this.server.tool('email_send_enhanced_templated', {
            ...
            try {
                ...
                if (customArgs)
                    emailData.customArgs = customArgs;
                if (sendAt)
                    emailData.sendAt = new Date(sendAt);
                if (sandboxMode !== undefined)
                    emailData.sandboxMode = sandboxMode;
        ...
        this.server.tool('email_schedule_templated', {
        ...
        this.server.tool('email_test_connection', {}, async () => {
            try {
                if (!process.env.EMAIL_SERVICE && !process.env.SMTP_HOST) {
                    return {
                        content: [{ type: 'text', text: 'Error: Email service configuration required. Set EMAIL_SERVICE or SMTP configuration environment variables.' }],
        ...
        this.server.tool('email_check_features', {}, async () => {
            try {
                if (!process.env.EMAIL_SERVICE && !process.env.SMTP_HOST) {
                    return {
                        content: [{ type: 'text', text: JSON.stringify({
        ...
        this.server.tool('sendgrid_create_dynamic_template', {
            name: z.string().describe('Template name'),
        ...
        this.server.tool('sendgrid_send_bulk_email', {
        ...
        this.server.tool('sendgrid_get_email_statistics', {
        ...
        this.server.tool('sendgrid_manage_suppression', {
            ...
            try {
                ...
                let result;
                if (action === 'list') {
                    result = await sendGridService.getSuppressions(suppressionType);
                }
                else if (action === 'add' && email && suppressionType) {
                    result = await sendGridService.addSuppression(email, suppressionType);
                }
                else if (action === 'remove' && email && suppressionType) {
                    result = await sendGridService.removeSuppression(email, suppressionType);
                }
                else {
        ...
        this.server.tool('sendgrid_validate_email', {
        ...
        this.server.tool('sendgrid_manage_contact_lists', {
        ...
        this.server.tool('register_user_with_automation', {
        ...
        this.server.tool('create_subscription_flow', {
        ...
        this.server.tool('process_payment_webhook_with_email', {
                    ...
                    try {
                        switch (eventType) {
                            ...
                            case 'invoice.payment_succeeded':
                                ...
                                try {
                                    const user = await this.pb.collection('users').getFirstListItem(`stripe_customer_id = '${eventData?.customer}'`);
        ...
        this.server.tool('setup_complete_saas_backend', {
        ...
        this.server.tool('cancel_subscription_with_email', {
            ...
            try {
                ...
                try {
                    const subscriptionRecord = await this.pb.collection('stripe_subscriptions').getFirstListItem(`stripe_subscription_id = '${subscriptionId}'`);
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (canceledSubscription.customer) {
                                const customer = await this.stripeService.retrieveCustomer(canceledSubscription.customer);
        ...
        this.server.tool('get_saas_backend_status', {
            ...
            try {
                ...
                if (includeRecommendations) {
                    ...
                    if (status.collections) {
                        const missingEssential = Object.entries(status.collections)
                            .filter(([name, info]) => info.is_essential && !info.exists)
                            .map(([name]) => name);
                    ...
                    if (!process.env.POCKETBASE_URL)
                        missingEnvVars.push('POCKETBASE_URL');
                    if (!process.env.APP_NAME)
                        missingEnvVars.push('APP_NAME (recommended)');
                    if (!process.env.SUPPORT_EMAIL)
                        missingEnvVars.push('SUPPORT_EMAIL (recommended)');
                ...
                const issues = [];
                if (!status.pocketbase?.connected)
                    issues.push('pocketbase');
                if (status.stripe?.configured && !status.stripe?.connected)
                    issues.push('stripe');
                if (status.email?.configured && !status.email?.connected)
                    issues.push('email');
        ...
        // Enhanced error handling with ClientResponseError patterns
        this.server.tool('pb_parse_error', {
        ...
        this.server.tool('pb_get_base_url', {}, async () => {
            try {
                return {
                    content: [{
                            type: 'text',
        ...
        this.server.tool('pb_safe_filter', {
        ...
        this.server.tool('pb_get_first_list_item', {
        ...
        this.server.tool('pb_health_check', {}, async () => {
            try {
                const healthStatus = {
                    timestamp: new Date().toISOString(),
                    status: 'checking',
        ...
        this.server.tool('pb_impersonate_with_duration', {
        ...
        this.server.tool('pb_truncate_collection', {
        ...
        this.server.tool('list_records_api', {
        ...
        this.server.tool('get_full_list_api', {
            collection: z.string().describe('Collection name to query records from'),
        ...
        this.server.tool('get_first_list_item_api', {
            collection: z.string().describe('Collection name to query records from'),
        ...
        this.server.tool('get_record_api', {
        ...
        // Update record by ID
        this.server.tool('update_record_api', {
        ...
        // Delete record by ID
        this.server.tool('delete_record_api', {
        ...
        this.server.tool('list_auth_methods_api', {
        ...
        this.server.tool('auth_with_password_api', {
            collection: z.string().describe('Auth collection name (e.g., "users")'),
        ...
        this.server.tool('request_otp_api', {
            collection: z.string().describe('Auth collection name (e.g., "users")'),
        ...
        this.server.tool('auth_with_otp_api', {
            collection: z.string().describe('Auth collection name (e.g., "users")'),
        ...
        this.server.tool('auth_refresh_api', {
            collection: z.string().describe('Auth collection name (e.g., "users")'),
            expand: z.string().optional().describe('Relations to expand in auth record'),
            fields: z.string().optional().describe('Specific fields to return in auth record')
        ...
        this.server.tool('request_password_reset_api', {
            collection: z.string().describe('Auth collection name (e.g., "users")'),
        ...
        this.server.tool('confirm_password_reset_api', {
            collection: z.string().describe('Auth collection name (e.g., "users")'),
        ...
        this.server.tool('request_verification_api', {
            collection: z.string().describe('Auth collection name (e.g., "users")'),
        ...
        this.server.tool('confirm_verification_api', {
            collection: z.string().describe('Auth collection name (e.g., "users")'),
        ...
        this.server.tool('list_collections_api', {
        ...
        this.server.tool('get_collection_api', {
        ...
        // === UTILITY API TOOLS ===
        // Health check
        this.server.tool('health_check_api', {}, async () => {
            try {
        ...
        this.server.tool('build_filter_expression', {
        ...
        this.server.tool('batch_records_api', {
                            ...
                            switch (request.method) {
                                ...
                                case 'PATCH':
                                    if (!request.id || !request.data)
                                        throw new Error('PATCH request requires id and data');
                                    result = await this.pb.collection(collection).update(request.id, request.data);
                                    break;
                                case 'DELETE':
                                    if (!request.id)
                                        throw new Error('DELETE request requires id');
        ...
        // Get file URL/info
        this.server.tool('get_file_url_api', {
            collection: z.string().describe('Collection name containing the record'),
        ...
        this.server.tool('realtime_subscription_info', {
...
{"clientId": "CLIENT_ID", "command": "subscribe", "data": {"topic": "${subscriptionTopic}"}}
        ...
        this.server.tool('advanced_query_builder', {
            ...
            try {
                ...
                if (sort_fields && sort_fields.length > 0) {
                    const sortParts = sort_fields.map(sort => `${sort.direction === 'DESC' ? '-' : '+'}${sort.field}`);
                ...
                let processedResults = result.items;
                if (groupBy) {
                    const grouped = {};
    ...
    async executeRuleAction(action, record, collection) {
        switch (action.type) {
            case 'email':
                if (this.emailService) {
    ...
    async executeWorkflowStep(step, instance, input) {
        try {
            switch (step.type) {
                case 'email':
                    if (this.emailService) {
                ...
                case 'delay':
                    ...
                    return { stepId: step.id, status: 'completed', type: step.type };
                case 'condition':
                    const conditionMet = this.evaluateCondition(input[step.config.field], step.config.operator, step.config.value);
    ...
    applyFilter(item, config) {
        ...
        if (config.condition) {
            return eval(config.condition.replace(/\$\{(\w+)\}/g, (_, field) => JSON.stringify(item[field])));
        }
        return true;
    }
    ...
    processGroupedMetrics(data, metrics, groupBy) {
        const groups = {};
        data.forEach(item => {
    ...
    parseSmitheryConfig(query) {
        ...
        if (config.pocketbaseUrl)
            flattened.pocketbaseUrl = config.pocketbaseUrl;
        if (config.pocketbase?.url)
            flattened.pocketbaseUrl = config.pocketbase.url;
        if (config.adminEmail)
            flattened.adminEmail = config.adminEmail;
        if (config.admin?.email)
            flattened.adminEmail = config.admin.email;
        if (config.adminPassword)
            flattened.adminPassword = config.adminPassword;
        if (config.admin?.password)
            flattened.adminPassword = config.admin.password;
        if (config.stripeSecretKey)
            flattened.stripeSecretKey = config.stripeSecretKey;
        if (config.stripe?.secretKey)
            flattened.stripeSecretKey = config.stripe.secretKey;
        if (config.emailService)
            flattened.emailService = config.emailService;
        if (config.email?.service)
            flattened.emailService = config.email.service;
        if (config.smtpHost)
            flattened.smtpHost = config.smtpHost;
        if (config.smtp?.host)
            flattened.smtpHost = config.smtp.host;
        if (config.sendgridApiKey)
            flattened.sendgridApiKey = config.sendgridApiKey;
        if (config.sendgrid?.apiKey)
            flattened.sendgridApiKey = config.sendgrid.apiKey;
        return flattened;
    }
    async formatAsPDF(data, title) {
    ...
    async runSSE(port = 3000, host = 'localhost', corsOrigin = '*') {
...
`);
    ...
    async runHTTP(port = 3000, host = 'localhost', corsOrigin = '*') {
    ...
    setupShutdownHandlers(transports) {
        process.on('SIGINT', async () => {
...
function detectTransportType() {
    ...
    if (process.env.MCP_TRANSPORT_TYPE) {
        const envType = process.env.MCP_TRANSPORT_TYPE;
...
async function startServer() {
    ...
    if (config.port)
        console.error(`[MCP DEBUG] Port: ${config.port}`);
    if (config.host)
        console.error(`[MCP DEBUG] Host: ${config.host}`);
    try {
        switch (config.type) {
            case 'stdio':
