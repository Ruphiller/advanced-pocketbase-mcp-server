...
export class PocketBaseServer {
    ...
    setupPrompts(server) {
        ...
        server.prompt("create-collection", "Create a new collection with specified fields", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
        ...
        server.prompt("create-record", "Create a new record in a collection", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
        ...
        server.prompt("build-query", "Build a query for a collection with filters, sorting, and expansion", async (extra) => ({
            messages: [{
                    role: "user",
                    content: {
                        type: "text",
    ...
    setupResources(server) {
        ...
        server.resource("server-info", "pocketbase://info", async (uri) => {
            try {
                return {
                    contents: [{
                            uri: uri.href,
        ...
        server.resource("collections", "pocketbase://collections", async (uri) => {
            try {
        ...
        // Auth info resource
        server.resource("auth-info", "pocketbase://auth", async (uri) => {
            try {
                return { contents: [{
                            uri: uri.href,
                            text: JSON.stringify({
    ...
    setupTools(server) {
        ...
        server.tool('get_collection_schema', {
        ...
        server.tool('stream_collection_changes', {
