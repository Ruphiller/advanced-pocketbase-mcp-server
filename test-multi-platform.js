...
try {
  ...
  // Check smithery config exists
  const smitheryConfigExists = fs.existsSync(path.join(__dirname, 'smithery.config.js'));
  tests.smithery.checks.push({
    name: 'Smithery build configuration',
    status: smitheryConfigExists ? 'PASS' : 'FAIL',
    details: smitheryConfigExists ? 'smithery.config.js exists' : 'Build config missing'
  });

  // Check TypeScript config for Smithery
  const tsConfigSmitheryExists = fs.existsSync(path.join(__dirname, 'tsconfig.smithery.json'));
  tests.smithery.checks.push({
    name: 'TypeScript configuration for Smithery',
    status: tsConfigSmitheryExists ? 'PASS' : 'FAIL',
    details: tsConfigSmitheryExists ? 'tsconfig.smithery.json exists' : 'TS config missing'
  });
